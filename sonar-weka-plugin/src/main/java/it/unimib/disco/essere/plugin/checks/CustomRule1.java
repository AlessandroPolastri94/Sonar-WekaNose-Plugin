...
@Rule(key = "CustomRule1", name = "Custom Rule 1", description = "Your Custom Rule.", tags = { "codesmells" })

public class CustomRule1 extends IssuableSubscriptionVisitor {
	...
	public List<Kind> nodesToVisit() {
		...
		System.out.println("[INFO] Reading Custom Rule 1 Prediction...");
		try {
			...
			for (String line : lines) {
				...
				if (bool.equalsIgnoreCase("true")) {
					...
					if (csvClass.equalsIgnoreCase(symbol.enclosingClass().name().toString())) {
						...
						lines.remove(line);
						reportIssue(_class.simpleName(), "Custom Rule 1 detected: " + predicted);
						break;
					}
				}
			}
