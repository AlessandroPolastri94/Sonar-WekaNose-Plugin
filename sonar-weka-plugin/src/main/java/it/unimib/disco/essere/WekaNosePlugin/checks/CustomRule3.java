...
@Rule(key = "CustomRule3", name = "Custom Rule 3", description = "Your Custom Rule.", tags = { "codesmells" })

public class CustomRule3 extends IssuableSubscriptionVisitor {
	...
	public List<Kind> nodesToVisit() {
		...
		System.out.println("[INFO] Reading Custom Rule 3 Prediction...");
		try {
					...
					if (methodClass.equalsIgnoreCase(symbol.enclosingClass().name().toString())) {
						...
						if (methodName.equalsIgnoreCase(method.simpleName().toString())) {
							...
							if (count == countDeleted) {
								...
								lines.remove(line);
								reportIssue(method.simpleName(), "Custom Rule 3 detected: " + predicted);
								break;
							}
						}
					}
			...
			for (String line : lines) {
				...
				if (bool.equalsIgnoreCase("true")) {
					...
					if (csvClass.equalsIgnoreCase(symbol.enclosingClass().name().toString())) {
						...
						lines.remove(line);
						reportIssue(_class.simpleName(), "Custom Rule 3 detected: " + predicted);
						break;
					}
				}
			}
