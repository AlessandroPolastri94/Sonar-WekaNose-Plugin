...
@Rule(key = "CustomRule7", name = "Custom Rule 7", description = "Your Custom Rule.", tags = { "codesmells" })

public class CustomRule7 extends IssuableSubscriptionVisitor {
	...
	public List<Kind> nodesToVisit() {
		...
		System.out.println("[INFO] Reading Custom Rule 7 Prediction...");
		try {
					...
					if (methodClass.equalsIgnoreCase(symbol.enclosingClass().name().toString())) {
						...
						if (methodName.equalsIgnoreCase(method.simpleName().toString())) {
							...
							if (count == countDeleted) {
								...
								lines.remove(line);
								reportIssue(method.simpleName(), "Custom Rule 7 detected: " + predicted);
								break;
							}
						}
					}
			...
			for (String line : lines) {
				...
				if (bool.equalsIgnoreCase("true")) {
					...
					if (csvClass.equalsIgnoreCase(symbol.enclosingClass().name().toString())) {
						...
						lines.remove(line);
						reportIssue(_class.simpleName(), "Custom Rule 7 detected: " + predicted);
						break;
					}
				}
			}
